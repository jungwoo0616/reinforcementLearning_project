# -*- coding: utf-8 -*-
# file: soccer_demo_dqn_vs_a2c.py
# 목적: BLUE(b0,b1: DQN) vs ORANGE(o2,o3: A2C) - best 모델 4개를 불러와 시청(watch) 데모 실행
# - 관측차원(336/338) 자동 보정: 모델이 기대하는 차원에 맞춰 one-hot(2) 부착/제거
# - Unity 타임아웃/포트 충돌 자동 재시도
# - DQN이 뱉는 Discrete 액션을 MultiDiscrete로 디코딩해서 Unity에 전달
#   (오렌지 A2C는 MultiDiscrete 그대로 사용)
...
# BLUE 팀 DQN
BLUE_B0 = os.path.join("..", "DQN", "models_dual_final_blue",   "dualdqn_latest")      # DQN BLUE b0
BLUE_B1 = os.path.join("..", "DQN", "models_dual_final_blue",   "dualdqn_latest")      # DQN BLUE b1

# ORANGE 팀 A2C
ORNG_O2 = os.path.join("..", "A2C", "models_dual_final_orange", "duala2c_latest")      # A2C ORANGE o2
ORNG_O3 = os.path.join("..", "A2C", "models_dual_final_orange", "duala2c_latest")      # A2C ORANGE o3
...
def make_demo_env_with_retries(
    ...
    worker_id=2468,
    time_scale=1.0,
    blue_team_name="BLUE (DQN b0,b1)",
    orange_team_name="ORANGE (A2C o2,o3)",
    max_retries=3,
    timeout_wait=180
):
...
def main():
    # 1) 모델 로드
    print("[LOAD] BLUE(DQN) b0:", BLUE_B0)
    print("[LOAD] BLUE(DQN) b1:", BLUE_B1)
    print("[LOAD] ORANGE(A2C) o2:", ORNG_O2)
    print("[LOAD] ORANGE(A2C) o3:", ORNG_O3)

    # BLUE: DQN, ORANGE: A2C
    b0 = DQN.load(BLUE_B0, env=None, device="cpu")
    b1 = DQN.load(BLUE_B1, env=None, device="cpu")
    o2 = A2C.load(ORNG_O2, env=None, device="cpu")
    o3 = A2C.load(ORNG_O3, env=None, device="cpu")
    ...
    for t in range(STEPS):
        ...
        # ----- 원시 액션 (모델 출력) -----
        # BLUE: DQN → Discrete action
        a0_raw, _ = b0.predict(obs0, deterministic=True)
        a1_raw, _ = b1.predict(obs1, deterministic=True)
        # ORANGE: A2C → MultiDiscrete 직접 사용
        a2_raw, _ = o2.predict(obs2, deterministic=True)
        a3_raw, _ = o3.predict(obs3, deterministic=True)

        # ----- 환경 action_space에 맞게 변환 -----
        if isinstance(action_space, spaces.MultiDiscrete):
            # BLUE(DQN): Discrete -> MultiDiscrete 디코딩
            a0 = dqn_to_multidiscrete(a0_raw, action_space)
            a1 = dqn_to_multidiscrete(a1_raw, action_space)
            # ORANGE(A2C): 이미 MultiDiscrete라고 가정 (학습 시와 동일)
