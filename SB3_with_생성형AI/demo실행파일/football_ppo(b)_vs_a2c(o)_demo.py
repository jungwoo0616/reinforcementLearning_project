...
# 목적: BLUE(b0,b1,PPO) vs ORANGE(o2,o3,A2C) - 각 팀의 best 모델 4개를 불러와 시청(watch) 데모 실행
...
# ※ 블루팀 경로는 PPO로 학습된 zip로 지정하세요.
BLUE_B0 = os.path.join("..", "PPO", "models_dual_final_blue",   "dualppo_latest.zip")
BLUE_B1 = os.path.join("..", "PPO", "models_dual_final_blue",   "dualppo_latest.zip")

# ※ 오렌지팀 경로는 A2C로 학습된 zip로 지정하세요.
ORNG_O2 = os.path.join("..", "A2C", "models_dual_final_orange", "duala2c_latest.zip")
ORNG_O3 = os.path.join("..", "A2C", "models_dual_final_orange", "duala2c_latest.zip")
...
def main():
    # 1) 모델 로드
    print("[LOAD] BLUE b0 (PPO):", BLUE_B0)
    print("[LOAD] BLUE b1 (PPO):", BLUE_B1)
    print("[LOAD] ORANGE o2 (A2C):", ORNG_O2)
    print("[LOAD] ORANGE o3 (A2C):", ORNG_O3)
    ...
    for t in range(STEPS):
        ...
        a0, _ = b0.predict(
            fix_obs_for_model(b0, np.asarray(obs[0], dtype=np.float32), default_onehot=0),
            deterministic=True
        )
        a1, _ = b1.predict(
            fix_obs_for_model(b1, np.asarray(obs[1], dtype=np.float32), default_onehot=1),
            deterministic=True
        )
        a2, _ = o2.predict(
            fix_obs_for_model(o2, np.asarray(obs[2], dtype=np.float32), default_onehot=0),
            deterministic=True
        )
        a3, _ = o3.predict(
            fix_obs_for_model(o3, np.asarray(obs[3], dtype=np.float32), default_onehot=1),
            deterministic=True
        )
