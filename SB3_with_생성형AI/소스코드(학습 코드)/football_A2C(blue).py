# -*- coding: utf-8 -*-
# file: soccer_train_dual_selfplay_6h_blue_safe_a2c.py
# 목적: Dual A2C Self-Play (BLUE 0/1) - 6시간 커리큘럼(안정성 강화: WinError1455 및 CUDA 초기화/포트 충돌 회피)
# - base_port 내부 합산 방식 유지(내부에서 base_port + worker_id)
# - DummyVecEnv(단일 프로세스), 스냅샷 CPU 로드(A2C 전용), 관측차원 자동 보정(+2 one-hot: 0/1 구분)
# - 환경 재빌드 시 graceful close + GC + 대기, 바인딩 실패/worker 충돌 시 블록 시프트+재시도
# - ★ 매 페이즈 종료 시 최신 모델을 models_dual_final_blue/duala2c_latest.zip 로 저장(갱신)
...
if __name__ == "__main__":
    ...
    FINAL_LATEST = os.path.join(FINAL_DIR, "duala2c_latest")  # ★ A2C 전용 파일명
    ...
    A2C_KW = dict(
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/a2c_dual_selfplay_6h_blue",
    )
    ...
    logger = configure("logs/a2c_blue", ["stdout", "csv", "tensorboard"])
    ...
    for phase_idx, ph in enumerate(CURRICULUM, start=1):
        ...
        while time.time() - t0 < minutes * 60:
            model.learn(
                ...
                callback=[eval_cb_b0, eval_cb_b1, pbar],
        ...
        if phase_idx % 2 == 0:
            ...
            snap_path = os.path.join(SNAP_DIR, f"duala2c_{stamp}")  # ★ A2C 전용 접두사
    ...
    print("[DONE] Dual A2C Self-Play (6h, BLUE, SAFE) finished.")
