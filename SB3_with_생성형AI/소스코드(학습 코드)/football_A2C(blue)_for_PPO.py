# -*- coding: utf-8 -*-
# file: blue_finetune_vs_orange3h.py
# 목적: BLUE(A2C) 3시간 추가학습 - ORANGE(PPO) 최신 전술에 대응하도록 미세조정
# - 블루 최신 모델(duala2c_latest.zip)에서 이어서 학습
# - 상대(오렌지 2/3)는 dualppo_latest.zip 스냅샷을 강하게 사용
# - 90분 x 2 페이즈(learn_id=0 → 1), 총 180분
# - 종료 시 models_dual_final_blue/duala2c_latest.zip 으로 원자적 저장
...
# BLUE = A2C (학습 대상)
# ORANGE = PPO (상대 전술)
BLUE_LATEST_PATH   = os.path.join("A2C/models_dual_final_blue",   "duala2c_latest.zip")      # 블루 최신(A2C)
ORANGE_LATEST_PATH = os.path.join("PPO/models_dual_final_orange", "dualppo_latest.zip")      # 오렌지 최신(PPO)

SNAP_DIR     = "snapshots_dual_blue_finetune"
BEST_DIR     = "best_models_dual_blue_finetune"
EVAL_LOG     = "eval_logs_dual_blue_finetune"
FINAL_DIR    = "models_dual_final_blue"
...
# ------------------ Self-Play 학습 환경 (BLUE 0/1 학습) ------------------
class SelfPlaySoccerEnv(gym.Env):
    metadata = {"render.modes": ["human", "rgb_array"]}

    def __init__(self,
                 learn_agent_id: int,             # 0 또는 1 (BLUE)
                 blue_latest: str,                # 블루 팀메이트(A2C)
                 orange_latest: str,              # 오렌지 상대(PPO)
                 weak_p_idle: float = 0.97,
                 opponent_probs: dict = None,     # 오렌지(2/3)
                 teammate_probs: dict = None,     # 블루(0/1)
                 render: bool = False,
        ...
        # 기본 확률: 상대는 ORANGE 최신(PPO)을 강하게 사용
        ...
        # 오렌지 상대(2/3)
        self._opponent2 = PolicyMixer(self.action_space, weak_p_idle=weak_p_idle,
                                      latest_path=orange_latest, pool_paths=[],
                                      probs=opponent_probs, seed=seed+2)
        self._opponent3 = PolicyMixer(self.action_space, weak_p_idle=weak_p_idle,
                                      latest_path=orange_latest, pool_paths=[],
    ...
    def step(self, action):
        ...
        # BLUE(0/1) 중 learn_id가 우리 학습 에이전트
        if self.learn_id == 0:
            a0 = aL
...
if __name__ == "__main__":
    ...
    FILE_NAME = None
    SEED = 888
    BASE_PORT = 9500         # 오렌지 쪽과 포트 분리
    START_WORKER = 1600
    N_ENVS = 6
    TIME_SCALE = 20.0

    # 3시간(90분×2) 커리큘럼: learn_id=0 → 1 (BLUE 0/1)
    CURRICULUM = [
        {"minutes": 90, "learn_id": 0, "weak_p_idle": 0.97},
        {"minutes": 90, "learn_id": 1, "weak_p_idle": 0.97},
    ]

    # 블루 팀메이트 / 오렌지 상대 확률
    TEAMMATE_BASE_PROBS = {"latest": 0.70, "pool": 0.00, "weak": 0.30}  # BLUE 최신(A2C)
    OPP_BASE_PROBS      = {"latest": 0.90, "pool": 0.00, "weak": 0.10}  # ORANGE 최신(PPO) 강하게

    # ===== VecEnv 빌더 =====
    def make_env_fn(learn_id, blue_latest, orange_latest, weak_p_idle,
    ...
    def build_vec_env(n_envs, start_worker_id, base_port,
                      learn_id, blue_latest, orange_latest, weak_p_idle, opp_probs, mate_probs,
                      file_name, seed, time_scale=20.0):
        env_fns = []
        for i in range(n_envs):
            wid = start_worker_id + i
            env_fns.append(
                make_env_fn(learn_id, blue_latest, orange_latest, weak_p_idle,
    ...
    A2C_KW = dict(
        ...
        gamma=0.995,
        gae_lambda=0.95,
        ent_coef=0.008,            # 오렌지 전술 대응 위해 약간 낮춰 exploitation↑
        vf_coef=0.5,
        max_grad_norm=0.5,
        use_rms_prop=True,
        rms_prop_eps=1e-5,
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/a2c_finetune_blue_vs_orange3h",
    )
    ...
    env = build_vec_env(
        ...
        learn_id=init["learn_id"], blue_latest=BLUE_LATEST_PATH, orange_latest=ORANGE_LATEST_PATH,
    ...
    # ===== 블루 최신(A2C) 모델 로드(필수) =====
    assert os.path.isfile(BLUE_LATEST_PATH), f"BLUE_LATEST_PATH not found: {BLUE_LATEST_PATH}"
    from stable_baselines3 import A2C
    model = A2C.load(BLUE_LATEST_PATH, env=env, device=DEVICE, **A2C_KW)
    model.set_env(env)

    # (선택) 간단한 평가 env와 콜백 (BLUE 0/1 평가)
    eval_env_b0 = build_eval_env(
        start_worker_id=START_WORKER + 999, base_port=BASE_PORT,
        learn_id=0, blue_latest=BLUE_LATEST_PATH, orange_latest=ORANGE_LATEST_PATH,
    ...
    eval_env_b1 = build_eval_env(
        ...
        learn_id=1, blue_latest=BLUE_LATEST_PATH, orange_latest=ORANGE_LATEST_PATH,
        weak_p_idle=0.95, opp_probs=OPP_BASE_PROBS, mate_probs=TEAMMATE_BASE_PROBS,
    ...
    for phase_idx, ph in enumerate(CURRICULUM, start=1):
        ...
        env = build_vec_env(
            ...
            learn_id=learn_id, blue_latest=BLUE_LATEST_PATH, orange_latest=ORANGE_LATEST_PATH,
        ...
        print(f"\n[PHASE] {phase_idx}/{len(CURRICULUM)}  learn_id={learn_id}  weak_p={weak_p:.3f}  minutes={minutes}  vs=ORANGE(PPO latest, p≈{OPP_BASE_PROBS['latest']})")
        t0 = time.time()
        ...
        snap_path = os.path.join(SNAP_DIR, f"duala2c_blue_finetune_{stamp}")
    ...
    try:
        ...
        pass
    try:
        eval_env_b0.close(); eval_env_b1.close()
    except Exception:
        pass
    ...
    print("[DONE] Blue A2C 3-hour finetune vs Orange PPO latest finished.")
