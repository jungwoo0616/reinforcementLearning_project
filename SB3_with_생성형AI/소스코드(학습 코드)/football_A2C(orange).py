# -*- coding: utf-8 -*-
# file: soccer_train_dual_selfplay_6h_orange_safe_a2c.py
# 목적: Dual A2C Self-Play (ORANGE 2/3) - 6시간 커리큘럼(안정성 강화: WinError1455 및 CUDA 초기화/포트 충돌 회피)
# - base_port 내부 합산 방식 유지(내부에서 base_port + worker_id)
# - DummyVecEnv(단일 프로세스), 스냅샷 CPU 로드(A2C 전용), 관측차원 자동 보정(+2 one-hot: 2/3 구분)
# - 환경 재빌드 시 graceful close + GC + 대기, 바인딩 실패/worker 충돌 시 블록 시프트+재시도
# - ★ 매 페이즈 종료 시 최신 모델을 models_dual_final_orange/duala2c_orange_latest.zip 로 저장(갱신)
...
# ------------------ 스냅샷 정책 (A2C만 CPU 로드 + 관측차원 자동보정) ------------------
class SnapshotPolicy:
    def __init__(self, path_zip: str, action_space: spaces.Space):
        os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")
        from stable_baselines3 import A2C
        self.model = A2C.load(path_zip, env=None, device="cpu")  # A2C 전용
        self.action_space = action_space
        self.exp_dim = None
        try:
...
# ------------------ Dual Self-Play 학습 환경 (ORANGE 2/3 학습) ------------------
...
if __name__ == "__main__":
    ...
    SEED = 42
    BASE_PORT = 9300
    START_WORKER = 640         # 동시 실행 안 하므로 기본값 사용
    N_ENVS = 6
    TIME_SCALE = 20.0
    ...
    FINAL_LATEST = os.path.join(FINAL_DIR, "duala2c_orange_latest")
    ...
    A2C_KW = dict(
        n_steps=max(8, (5 * 256) // N_ENVS),  # env 수 기준 ~256 스텝 롤아웃
        learning_rate=lr_schedule,
        gamma=0.995,
        gae_lambda=0.95,               # A2C에서도 GAE 사용
        ent_coef=0.01,
        vf_coef=0.5,
        max_grad_norm=0.5,
        use_rms_prop=True,
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/a2c_dual_selfplay_6h_orange",
    )
    ...
    model = A2C("MlpPolicy", env, **A2C_KW)

    # ---- 콘솔/CSV/텐서보드 로깅 설정 + 진행바 ----
    logger = configure("logs/a2c_orange", ["stdout", "csv", "tensorboard"])
    ...
    for phase_idx, ph in enumerate(CURRICULUM, start=1):
        ...
        while time.time() - t0 < minutes * 60:
            model.learn(
                ...
                callback=[eval_cb_o2, eval_cb_o3, pbar],
                log_interval=1   # 매 롤아웃마다 콘솔/CSV/TB 로깅
            )
        ...
        if phase_idx % 2 == 0:
            ...
            snap_path = os.path.join(SNAP_DIR, f"duala2c_orange_{stamp}")
    ...
    print("[DONE] Dual A2C Self-Play (6h, ORANGE, SAFE) finished.")
