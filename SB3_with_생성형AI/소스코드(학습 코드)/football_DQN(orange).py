# -*- coding: utf-8 -*-
# file: soccer_train_dual_selfplay_6h_orange_safe_dqn.py
# 목적: Dual DQN Self-Play (ORANGE 2/3) - 6시간 커리큘럼(안정성 강화 그대로)
# - 핵심 구조/로직/커리큘럼/스냅샷/포트 처리 전부 동일
# - PPO -> DQN 교체
# - DQN은 Discrete만 지원하므로 "학습자(오렌지 2/3) 액션"만 Discrete로 플래튼 옵션 추가(원 환경은 그대로 MultiDiscrete 허용)
# - 데모 스크립트 호환을 위해 최종 저장 파일명은 models_dual_final_orange/dualppo_latest.zip 로 유지
...
# ------------------ Dual Self-Play 학습 환경 (ORANGE 2/3 전용) ------------------
class SelfPlaySoccerEnvOrange(gym.Env):
    ...
    def __init__(self,
                 ...
                 worker_id: int = 0,
                 base_port: int = 6005,
                 flatten_for_dqn: bool = False):    # ★ 학습자 액션 Discrete 플래튼
        super().__init__()
        assert learn_agent_id in (2, 3), "Orange team learn_agent_id must be 2 or 3"
        ...
        # 관측 공간(+2 one-hot) — 오렌지 팀 구분 one-hot은 [0,1]
        ...
        # 오렌지 팀 학습 시: 팀메이트는 (2,3) 중 다른 ID, 상대는 0/1
        ...
        self._opponent0 = PolicyMixer(self.orig_action_space, weak_p_idle=weak_p_idle,
                                      latest_path=latest_snapshot, pool_paths=pool_snapshots,
                                      probs=opponent_probs, seed=seed+2)
        self._opponent1 = PolicyMixer(self.orig_action_space, weak_p_idle=weak_p_idle,
                                      latest_path=latest_snapshot, pool_paths=pool_snapshots,
...
if __name__ == "__main__":
    ...
    FINAL_LATEST = os.path.join(FINAL_DIR, "dualppo_latest")  # 데모 호환 위해 파일명 유지
    ...
    def make_env_fn(learn_id, latest_snapshot, pool_paths, weak_p_idle, opp_probs, mate_probs,
        ...
        def _init():
            return SelfPlaySoccerEnvOrange(
    ...
    DQN_KW = dict(
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/dqn_dual_selfplay_6h_orange",
    )
    ...
    print("[DONE] Dual DQN Self-Play (6h, ORANGE, SAFE) finished.")
