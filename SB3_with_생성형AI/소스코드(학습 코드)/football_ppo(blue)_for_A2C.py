# -*- coding: utf-8 -*-
# file: soccer_finetune_blue_vs_orange3h.py
# 목적: BLUE(PPO) 3시간 추가학습 - ORANGE(A2C) 최신 전술에 대응하도록 미세조정
# - 블루 최신 모델(dualppo_latest.zip)로부터 이어서 학습
# - 상대(오렌지 2/3)는 duala2c_latest.zip 스냅샷을 강하게 사용
# - 벡터환경 DummyVecEnv, 관측차원 +2(one-hot: 0/1) 자동 보정
# - 90분 x 2 페이즈(learn_id=0 → 1), 총 180분
# - 학습 중 EvalCallback(선택), 종료 시 최신 모델 원자적 저장
...
ORANGE_LATEST_PATH = os.path.join("A2C/models_dual_final_orange", "duala2c_orange_latest.zip")   # 오렌지 최신 A2C

# ===== 출력/스냅샷 디렉토리 =====
SNAP_DIR   = "snapshots_dual_blue_finetune"
BEST_DIR   = "best_models_dual_blue_finetune"
EVAL_LOG   = "eval_logs_dual_blue_finetune"
FINAL_DIR  = "models_dual_final_blue"   # 저장 대상
FINAL_LATEST = os.path.join(FINAL_DIR, "dualppo_latest")  # .zip 자동 부착
...
# ------------------ Dual Self-Play 학습 환경 (BLUE 0/1 학습) ------------------
class SelfPlaySoccerEnv(gym.Env):
    metadata = {"render.modes": ["human", "rgb_array"]}

    def __init__(self,
                 learn_agent_id: int,             # 0 또는 1
                 blue_latest: str,                # 블루 팀메이트/풀(선택)
                 orange_latest: str,              # 오렌지 상대
        ...
        # 기본 확률: 상대는 orange_latest를 강하게 사용
        ...
        # 블루 팀메이트(0↔1) - latest는 BLUE_LATEST_PATH
        ...
        # 오렌지 상대(2/3) - latest는 ORANGE_LATEST_PATH
...
if __name__ == "__main__":
    ...
    # ===== 3시간 커리큘럼: 90분(learn_id=0) → 90분(learn_id=1) =====
    CURRICULUM = [
        {"minutes": 90, "learn_id": 0, "weak_p_idle": 0.97},
        {"minutes": 90, "learn_id": 1, "weak_p_idle": 0.97},
    ]

    # 팀메이트/상대 확률(위 SelfPlaySoccerEnv 기본값과 동일, 필요시 조정)
    TEAMMATE_BASE_PROBS = {"latest": 0.70, "pool": 0.00, "weak": 0.30}
    OPP_BASE_PROBS      = {"latest": 0.90, "pool": 0.00, "weak": 0.10}
    ...
    PPO_KW = dict(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.008,                # 약간 낮춰 exploitation 강화
        policy_kwargs=dict(net_arch=[512, 256]),
        verbose=1,
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/ppo_finetune_blue_vs_orange3h",
    )
    ...
    model = PPO.load(BLUE_LATEST_PATH, env=env, device=DEVICE, print_system_info=False)
    model.set_env(env)
    ...
    for phase_idx, ph in enumerate(CURRICULUM, start=1):
        ...
        print(f"\n[PHASE] {phase_idx}/{len(CURRICULUM)}  learn_id={learn_id}  weak_p={weak_p:.3f}  minutes={minutes}  vs=ORANGE(A2C latest, p≈{OPP_BASE_PROBS['latest']})")
        t0 = time.time()
        ...
        snap_path = os.path.join(SNAP_DIR, f"dualppo_finetune_{stamp}")
    ...
    print("[DONE] Blue PPO 3-hour finetune vs Orange A2C latest finished.")
