# -*- coding: utf-8 -*-
# file: soccer_train_dual_selfplay_6h_orange_safe.py
# 목적: Dual PPO Self-Play (ORANGE 2/3) - 6시간 커리큘럼(안정성 강화: WinError1455 및 CUDA 초기화/포트 충돌 회피)
...
# - ★ 매 페이즈 종료 시 최신 모델을 models_dual_final_orange/dualppo_latest.zip 로 저장(갱신)
# - ★ 커스텀 보상: 패스 보상(pass_reward) + 점유율 보상(possession_reward) 추가
...
# ------------------ Dual Self-Play 학습 환경 (ORANGE 2/3 학습자) ------------------
class SelfPlaySoccerEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 possession_reward: float = 0.005  # ORANGE가 공을 소유한 스텝당 보상
                 ):
        super().__init__()
        assert learn_agent_id in (2, 3)   # ★ ORANGE 팀
        ...
        # 팀 내부 식별(one-hot 2칸: ORANGE 2 vs ORANGE 3)
    ...
    def _compute_shaping_reward(self, info_d) -> float:
        """
        - ORANGE가 공을 소유한 스텝마다 작은 보상(점유율)
        - 같은 팀(ORANGE) 내에서 플레이어가 바뀌면 패스로 간주하고 추가 보상
        """
        shaped = 0.0
        ...
        # 1) 점유율 보상: ORANGE 팀이 공을 가지고 있으면 매 스텝마다 +possession_reward
        if team == self._ORANGE_TEAM_IDX:
            shaped += self.possession_reward

        # 2) 패스 보상: 직전 소유자와 현재 소유자가 같은 팀(ORANGE)이면서, 서로 다른 플레이어일 때
        if (self._last_ball_team == self._ORANGE_TEAM_IDX and
            team == self._ORANGE_TEAM_IDX and
...
if __name__ == "__main__":
    ...
    POSSESSION_REWARD = 0.005 # ORANGE가 공을 소유한 스텝당 보상

    # ★ 오렌지 버전 저장 경로/이름
    SNAP_DIR = "snapshots_dual_orange"
    BEST_DIR = "best_models_dual_orange"
    EVAL_LOG = "eval_logs_dual_orange"
    FINAL_DIR = "models_dual_final_orange"   # 최신 모델 저장 디렉터리
    FINAL_LATEST = os.path.join(FINAL_DIR, "dualppo_latest")  # dualppo_latest.zip 로 갱신

    os.makedirs(SNAP_DIR, exist_ok=True)
    os.makedirs(os.path.join(BEST_DIR, "o2"), exist_ok=True)   # ORANGE-2 best
    os.makedirs(os.path.join(BEST_DIR, "o3"), exist_ok=True)   # ORANGE-3 best
    ...
    CURRICULUM = [
        # Warmup (40)
        {"minutes": 8, "learn_id": 2, "weak_p_idle": 0.995},
        {"minutes": 8, "learn_id": 3, "weak_p_idle": 0.992},
        {"minutes": 8, "learn_id": 2, "weak_p_idle": 0.990},
        {"minutes": 8, "learn_id": 3, "weak_p_idle": 0.988},
        {"minutes": 8, "learn_id": 2, "weak_p_idle": 0.985},
        # Mid (200)
        {"minutes": 20, "learn_id": 3, "weak_p_idle": 0.975},
        {"minutes": 20, "learn_id": 2, "weak_p_idle": 0.975},
        {"minutes": 20, "learn_id": 3, "weak_p_idle": 0.970},
        {"minutes": 20, "learn_id": 2, "weak_p_idle": 0.970},
        {"minutes": 20, "learn_id": 3, "weak_p_idle": 0.968},
        {"minutes": 20, "learn_id": 2, "weak_p_idle": 0.968},
        {"minutes": 20, "learn_id": 3, "weak_p_idle": 0.965},
        {"minutes": 20, "learn_id": 2, "weak_p_idle": 0.965},
        {"minutes": 20, "learn_id": 3, "weak_p_idle": 0.963},
        {"minutes": 20, "learn_id": 2, "weak_p_idle": 0.963},
        # Sparring (60)
        {"minutes": 10, "learn_id": 2, "weak_p_idle": 0.955},
        {"minutes": 10, "learn_id": 3, "weak_p_idle": 0.955},
        {"minutes": 10, "learn_id": 2, "weak_p_idle": 0.950},
        {"minutes": 10, "learn_id": 3, "weak_p_idle": 0.950},
        {"minutes": 10, "learn_id": 2, "weak_p_idle": 0.948},
        {"minutes": 10, "learn_id": 3, "weak_p_idle": 0.948},
        # Finish (60) — 2 비중 소폭↑
        {"minutes": 15, "learn_id": 2, "weak_p_idle": 0.945},
        {"minutes": 15, "learn_id": 3, "weak_p_idle": 0.940},
        {"minutes": 15, "learn_id": 2, "weak_p_idle": 0.938},
        {"minutes": 15, "learn_id": 3, "weak_p_idle": 0.935},
    ]
    ...
    PPO_KW = dict(
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/ppo_dual_selfplay_6h_orange",
    )
    ...
    eval_env_o2 = build_eval_env(
        ...
        learn_id=2, latest_snapshot=latest_snapshot, pool_paths=pool_snapshots,
    ...
    eval_env_o3 = build_eval_env(
        ...
        learn_id=3, latest_snapshot=latest_snapshot, pool_paths=pool_snapshots,
        weak_p_idle=0.95, opp_probs={"latest":0.10,"pool":0.10,"weak":0.80}, mate_probs=TEAMMATE_BASE_PROBS,
    ...
    o2_cand = os.path.join(BEST_DIR, "o2", "best_model.zip")
    o3_cand = os.path.join(BEST_DIR, "o3", "best_model.zip")
    if os.path.exists(o2_cand): print(f"[BEST-o2] {o2_cand}")
    if os.path.exists(o3_cand): print(f"[BEST-o3] {o3_cand}")
    print("[DONE] Dual PPO Self-Play (6h, ORANGE, SAFE, with pass/possession rewards) finished.")
