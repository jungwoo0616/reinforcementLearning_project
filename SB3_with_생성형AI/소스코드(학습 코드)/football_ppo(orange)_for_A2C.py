# -*- coding: utf-8 -*-
# file: soccer_finetune_orange_vs_blue3h.py
# 목적: ORANGE(PPO) 3시간 추가학습 - BLUE(A2C) 최신 전술에 대응하도록 미세조정
# - 오렌지 최신 모델(dualppo_latest.zip)로부터 이어서 학습
# - 상대(블루 0/1)는 duala2c_latest.zip 스냅샷을 강하게 사용
# - 벡터환경 DummyVecEnv, 관측차원 +2(one-hot: 2/3) 자동 보정
...
FINAL_DIR  = "models_dual_final_orange"   # 저장 대상
...
class SnapshotPolicy:
    ...
    def _with_onehot(obs_vec: np.ndarray, which: int = 0):
        # which는 여기서는 사용 안 함(기본 0팀 기준), dim 맞추기용
...
class SelfPlaySoccerEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 orange_latest: str,              # 오렌지 팀메이트/풀(PPO)
                 blue_latest: str,                # 블루 상대(A2C)
        ...
        # 기본 확률: 상대는 blue_latest(A2C)를 강하게 사용
        ...
        # 오렌지 팀메이트(2↔3) - latest는 ORANGE_LATEST_PATH(PPO)
        ...
        # 블루 상대(0/1) - latest는 BLUE_LATEST_PATH(A2C)
    ...
    def step(self, action):
        ...
        # ORANGE(2/3) 중 learn_id가 우리 학습 에이전트
        if self.learn_id == 2:
            a2 = aL
...
if __name__ == "__main__":
    ...
    # ===== 1시간 커리큘럼: 90분(learn_id=2) → 90분(learn_id=3) =====
    CURRICULUM = [
        {"minutes": 30, "learn_id": 2, "weak_p_idle": 0.97},
        {"minutes": 30, "learn_id": 3, "weak_p_idle": 0.97},
    ]

    # 팀메이트/상대 확률
    TEAMMATE_BASE_PROBS = {"latest": 0.85, "pool": 0.00, "weak": 0.15}  # ORANGE(PPO) 팀메이트
    OPP_BASE_PROBS      = {"latest": 0.90, "pool": 0.00, "weak": 0.10}  # BLUE(A2C) 상대 강하게
    ...
    PPO_KW = dict(
        ...
        seed=SEED,
        device=DEVICE,
        tensorboard_log="runs/ppo_finetune_orange_vs_blue3h",
    )
    ...
    model = PPO.load(ORANGE_LATEST_PATH, env=env, device=DEVICE, print_system_info=False)
    model.set_env(env)

    # (선택) 간단한 평가 env와 콜백 (ORANGE 2/3 평가)
    ...
    for phase_idx, ph in enumerate(CURRICULUM, start=1):
        ...
        print(f"\n[PHASE] {phase_idx}/{len(CURRICULUM)}  learn_id={learn_id}  weak_p={weak_p:.3f}  minutes={minutes}  vs=BLUE(A2C latest, p≈{OPP_BASE_PROBS['latest']})")
        t0 = time.time()
        ...
        snap_path = os.path.join(SNAP_DIR, f"dualppo_orange_finetune_{stamp}")
    ...
    print("[DONE] Orange PPO 1-hour finetune vs Blue A2C latest finished.")
